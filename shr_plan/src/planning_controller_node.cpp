...
public:
    ...
    // low level only called within protocol
    ...
    TRUTH_VALUE already_took_medicine(TRUTH_VALUE val, MedicineProtocol m) const override {
        ...
        if (auto index = get_inst_index(m, params)) {
            if (world_state_converter->get_world_state_msg()->person_taking_medicine == 1 &&
                compare_time_with_range(params.pddl.MedicineProtocols.take_medication_times[index.value()], 20)) {
                return TRUTH_VALUE::TRUE;
            }
            return val;
        }
...
private:
    ...
    bool compare_time_with_range(std::string param_time, int offset_minutes) const {
        auto msg = world_state_converter->get_world_state_msg();
        auto time = msg->time;
        std::stringstream ss(param_time);
        std::string time_1;
        ...
        const int offset_in_secs = offset_minutes * 60;

        // Calculate the start of the offset (offset minutes before start time)
        auto start_time_with_offset_secs = time_1_secs - offset_in_secs;

        const int second_in_day = 60 * 60 * 24;  // Total seconds in a day
        double clock_distance = fmod((time_2_secs - start_time_with_offset_secs + second_in_day), second_in_day);
        double time_to_check_normalized = fmod((time.sec - start_time_with_offset_secs + second_in_day), second_in_day);
        // Check if within 20 minutes from start time
        return time_to_check_normalized <= clock_distance;
    }
